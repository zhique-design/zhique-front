...
export const dynamicWrapper = ({
  ...
  dynamic({
    ...
    models: () =>
      models
        .filter((model) => modelNotExisted(window.dvaApp, model))
...
const RouteMap = (routeList) =>
  routeList.map(({ path, component, redirect, children, ...dynamics }) => {
    if (redirect) {
    ...
    if (children) {
      ...
      return (
        <Route
          ...
          render={(props) => (
            <Component {...props}>
              <Switch>{RouteMap(children)}</Switch>
            </Component>
          )}
        />
      );
...
const ZhiQue: React.FC<ConnectedRouterProps<{}>> = (props) => (
  <ConnectedRouter {...props}>
    <Switch>{RouteMap(routers)}</Switch>
  </ConnectedRouter>
);
